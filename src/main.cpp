...
void printMenu() { // функция, чисто чтоб отрисовать менюшку
    ...
    std::cout << "8. Удалить Ресурс (из корня)\n"; // пункт 8: удаление
    std::cout << "9. Поиск файлов по расширению\n"; // пункт 9: быстрый поиск
    std::cout << "10. Выход\n"; // валим
...
int main() { // точка входа в прогу
    ...
    while (true) { // бесконечный цикл (пока на 8 не нажмут)
        ...
        try { // открываем блок защиты от ошибок (эксепшенов)
            ...
            } else if (choice == 8) { // если 8 (удалить)
                std::string name;
                int level;
                std::cout << "Введите имя файла/папки для удаления: "; std::cin >> name;
                std::cout << "Ваш уровень прав (0-Гость, 1-Юзер, 2-Админ): ";
                if (!(std::cin >> level) || level < 0 || level > 2) {
                    std::cin.clear(); std::cin.ignore(10000, '\n'); // чистим
                    throw FileSystemException("Неверный уровень прав для операции!"); // кидаем ошибку
                }
                if (root->removeResource(name, static_cast<AccessLevel>(level))) {
                    std::cout << "Успешно удалено!\n";
                } else {
                    std::cout << "Ресурс с таким именем не найден в корневой папке.\n";
                }
            } else if (choice == 9) { // если 9 (поиск по формату)
                std::string ext;
                std::cout << "Формат для поиска (например txt): "; std::cin >> ext;
                SearchCriteria crit;
                crit.extension = ext; // задаем критерий поиска
                auto results = SearchAnalytics::findResources(root.get(), crit); // ищем по всему дереву
                std::cout << "Найдено файлов с таким расширением: " << results.size() << "\n";
                for (auto res : results) { // выводим красивым списком
                    if (File* f = dynamic_cast<File*>(res)) {
                        std::cout << " -> " << f->getName() << "." << f->getExtension() << " (" << f->getSize() << " байт)\n";
                    }
                }
            } else if (choice == 10) { // если 10 (выход)
                break; // вырываемся из бесконечного цикла
            } else { // если чушпан ввел 11+
